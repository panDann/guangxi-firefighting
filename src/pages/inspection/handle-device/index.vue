    ...
    <div class="gx-form-card">
        <div class="gx-form-card-body">
            <div>
                ...
                <div class="gx-form-card-list clearfix">
                    <ImgPick :imgUrls="imgUrls"
                        @on-image="onImage"
                        :title="`现场照片（${imgUrls.length}/3）`"/>
                </div>
            </div>
        </div>
    </div>
...
import ImgPick from '@/components/img-picker/index.vue'
export default {
  components: {
    ImgPick,
  },
  ...
  methods: {
      ...
      async complete(){
        ...
        let resDetails = await Api.updatePortalTask(loginInfo.token, that.deviceId, that.handling_type, that.handling_desc)
